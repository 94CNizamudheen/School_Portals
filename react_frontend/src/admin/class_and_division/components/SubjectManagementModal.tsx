...
const SubjectManagementModal: React.FC<SubjectManagementModalProps> = ({isOpen,  onClose,division,  onSubjectUpdate}) => {
  // const dispatch = useDispatch<AppDispatch>();
  const [newSubject, setNewSubject] = useState('');
  const [subjects, setSubjects] = useState<string[]>(division?.subjects || []);
  ...
  const handleSave = async () => {
    if (division) {
      try {
        // Dispatch your update subjects action here
        // dispatch(updateDivisionSubjects({ divisionId: division._id, subjects }));
        onSubjectUpdate();
        onClose();
      } catch (error) {
